...
export function runCorporationCycle(ns, options = {}) {
    ...
    const startupStatus = getMaterialWaveStatus(ns, cfg.agriculture, cfg.cities, cfg.agricultureStartupMaterials);
    const round1Status = getMaterialWaveStatus(ns, cfg.agriculture, cfg.cities, cfg.agricultureRound1Materials);
    ...
    return state("running", `Corporation manager active: ${stage}.`, cfg, actions, {
        ...
        materialWave: {
            ...
            agricultureRound2: getMaterialWaveStatus(ns, cfg.agriculture, cfg.cities, cfg.agricultureRound2Materials),
        },
    });
}
...
function ensureAgriculture(ns, cfg, actions) {
    ...
    safeCall(() => corp.unlockUpgrade("Smart Supply"), false);
...
function ensureTobacco(ns, cfg, actions) {
    ...
    for (const city of cfg.cities) {
        ...
        ensureOfficeGrowth(ns, cfg.tobacco, [city], city === cfg.mainCity ? 51 : 9, cfg.growthJobs, actions);
...
function ensureMaterialWave(ns, division, cities, materials, actions) {
    ...
    for (const city of cities) {
        for (const [material, rate, targetQty] of materials) {
            ...
            actions.push(`Buying ${material} in ${division}/${city}: ${formatQty(qty)} / ${formatQty(targetQty)}.`);
        }
    }
}
...
function manageTobaccoProducts(ns, cfg, actions) {
    ...
    for (const product of active.filter(p => Number(p.developmentProgress) >= 100)) {
        safeCall(() => corp.sellProduct(cfg.tobacco, cfg.mainCity, product.name, "MAX", "MP", true), false);
    }
    ...
    if (safeCall(() => corp.makeProduct(cfg.tobacco, cfg.mainCity, name, spend, spend), false) !== false) {
    ...
    return {
        ...
        offices: cfg.cities.map(city => {
            ...
            return {
                ...
                employees: office?.numEmployees ?? 0,
