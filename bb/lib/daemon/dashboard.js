...
function printResetCompact(ns, c, state) {
    const plan = state.resetPlan;

    if (!plan || !plan.updatedAt) {
        ns.print(
            `${section(c, "RESET")} ` +
            `${badge(c, "PLAN", "NONE", c.gray)}`
        );
        return;
    }
    ...
    ns.print(
        `${section(c, "RESET")} ` +
        `${badge(c, "READY", yesNo(plan.ready), plan.ready ? c.green : c.yellow)} ` +
        `${badge(c, "ARMED", yesNo(plan.armed), plan.armed ? c.green : c.red)} ` +
        `${badge(c, "PENDING", plan.pendingCount ?? 0, plan.pendingCount > 0 ? c.green : c.gray)} ` +
        `${badge(c, "INSTALLED", plan.installedCount ?? 0, c.cyan)}`
    );

    if (plan.reason) {
        ns.print(`${c.gray}${shorten(plan.reason, 110)}${c.reset}`);
    }
}
