...
import { filterTargetsByMode } from "./target-tiers";
...
export function getBestMoneyTarget(ns, servers) {
    const cleanServers = filterTargetsByMode(
        ns,
...
export function getBestExpTarget(ns, servers) {
    ...
    return pool
        .sort((a, b) => scoreExpTarget(ns, b) - scoreExpTarget(ns, a))[0] ?? "joesguns";
}
...
export function scoreExpTarget(ns, server) {
    ...
    const timePenalty = Math.pow(timeSeconds, 0.65);
...
export function getSecondaryMoneyTarget(
    ...
    const EARLY_GAME_TRASH = new Set([
        "n00dles",
        "foodnstuff",
        "sigma-cosmetics",
        "joesguns",
        ...
        candidates.length > 0
            ...
            : cleanServers
                ...
                .filter(server => {
                    ...
                    return !EARLY_GAME_TRASH.has(server);
                });
