...
export async function runServerPurchaser(ns, policy = {}) {
    ...
    const desiredRamRaw = getBestAffordableRam(ns, budget, maxRam);
    const desiredRam = applyUsefulRamFloor(ns, desiredRamRaw, owned.length, budget, maxRam);
...
function upgradeWeakestServer(ns, owned, desiredRam, safeSpendable) {
    ...
    while (targetRam > weakest.ram) {
        ...
        if (
            ...
            const upgraded = ns.cloud.upgradeServer(
                weakest.name,
                targetRam
            );
        ...
        targetRam = Math.floor(targetRam / 2);
    }
...
function applyUsefulRamFloor(ns, desiredRam, ownedCount, budget, maxRam) {
    if (ownedCount < MIN_SERVERS_BEFORE_RAM_FLOOR) {
        return desiredRam;
    }

    if (desiredRam >= MIN_USEFUL_SERVER_RAM) {
        return desiredRam;
    }

    const floorRam = Math.min(MIN_USEFUL_SERVER_RAM, maxRam);
    const floorCost = ns.cloud.getServerCost(floorRam);
