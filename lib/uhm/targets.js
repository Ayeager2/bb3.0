...
export function getValidTargetOrFallback(ns, rootedServers, target, mode, hosts = []) {
    ...
    const targetPlan = getBatchPlan(ns, target, "money");
...
export function getBestAffordableMoneyTarget(ns, servers, hosts = []) {
    ...
    return [...servers]
        ...
        .filter(server => safeGetServerGrowth(ns, server) > 0)
        .map(server => {
            const plan = getBatchPlan(ns, server, "money");
            if (!plan) return null;
            ...
            return {
                ...
                plan,
                affordability,
                score: scoreTarget(ns, server) * Math.min(1, affordability / 2),
            };
        })
        .filter(x => x && x.affordability >= 1.25)
